...
const AddMaintenanceDialog: React.FC = () => {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    try {
      addMaintenance(values as Omit<Maintenance, 'id'>);
      form.reset();
      setIsOpen(false);
    } catch (error) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] glass rounded-2xl animate-scaleIn">
        <DialogHeader>
          ...
          <DialogDescription>
            Remplissez les détails de la maintenance ci-dessous.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 py-4">
            ...
            />
            <Button type="submit" className="w-full mt-4 hover:animate-hover-lift gradient-brand text-primary-foreground">Ajouter la maintenance</Button>
          </form>
        </Form>
      </DialogContent>
    </Dialog>
