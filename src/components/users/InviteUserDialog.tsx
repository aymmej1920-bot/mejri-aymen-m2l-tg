...
const InviteUserDialog: React.FC = () => {
  ...
  const { inviteUser, roles } = useFleet();
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      await inviteUser(values.email, values.roleName, values.firstName || undefined, values.lastName || undefined);
      form.reset();
      setIsOpen(false);
    } catch (error) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] glass rounded-2xl animate-scaleIn">
        <DialogHeader>
          ...
          <DialogDescription>
            Envoyez une invitation par e-mail pour qu'un nouvel utilisateur rejoigne votre flotte.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
